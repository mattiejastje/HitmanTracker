...
_T.struct   = function(name) return { category="struct", name=name } end
_T.ptr      = function(type_def) return { category="ptr", type_def=type_def } end
_T.weak_ptr = function(type_def) return { category="weak_ptr", type_def=type_def } end
_T.vector   = function(element_type_def) return { category="vector", type_def=element_type_def } end
...
local _basic_readers = {
	i8    = function(addr) return readByte(addr, true) end,
	i16   = function(addr) return readShortInteger(addr, true) end,
	i32   = function(addr) return readInteger(addr, true) end,
	float = readFloat,
}
...
local function _type_size(type_def)
    ...
    elseif type_def.category == "ptr" or type_def.category == "weak_ptr" or type_def.category == "vector" or type_def.category == "circular_list" then
...
local function _read(addr, type_def)
    ...
    if not handler then error("Unknown type category: ".. type_def.category) end
    return handler(addr, type_def)
end

local function _validate(type_def)
    local validator = _validators[type_def.category]
    if not validator then error("Unknown type category: " .. type_def.category) end
    validator(type_def)
end
...
local function _read_pointer(addr)
    ...
    if p == 0 then return nil end  -- for consistency
    return p
end

_handlers.basic = function(addr, type_def)
    local reader = _basic_readers[type_def.name]
    if not reader then error("Unknown basic type: " .. type_def.name) end
    return reader(addr)
end
...
_handlers.struct = function(addr, type_def)
    ...
    if not def then error("Unknown struct: " .. name) end
...
_validators.struct = function(type_def)
    ...
    local def = _structs[name]
    if not def then error("Unknown struct: " .. name) end
    if not def.fields then error("Struct '" .. name .. "' has no fields") end
    if not def.size then error("Struct '" .. name .. "' has no size") end
    for _, field in ipairs(def.fields) do
        if not field.name then error("Struct '" .. name .. "' has a field with no name") end
        if not field.offset then error("Struct '" .. name .. "' field '" .. field.name .. "' has no offset") end
        if not field.type_def then error("Struct '" .. name .. "' field '" .. field.name .. "' has no type_def") end
        _validate(field.type_def)
    end
end

_validators.ptr = function(type_def)
    if not type_def.type_def then error("ptr has no type_def") end
    _validate(type_def.type_def)
end

_validators.weak_ptr = function(type_def)
    if not type_def.type_def then error("weak_ptr has no type_def") end
    _validate(type_def.type_def)
end

_validators.vector = function(type_def)
    if not type_def.type_def then error("vector has no type_def") end
    _validate(type_def.type_def)
end

_validators.circular_list = function(type_def)
    if not type_def.type_def then error("circular_list has no type_def") end
    if not type_def.next_field then error("circular_list has no next_field") end
    _validate(type_def.type_def)
    ...
    for _, field in ipairs(def.fields) do
        if field.name == type_def.next_field then
            if field.type_def.category ~= "weak_ptr" then
                error("circular_list next_field '" .. type_def.next_field .. "' in struct '" .. sub_name .. "' must be a weak_ptr, got: " .. field.type_def.category)
            end
			local next_type = field.type_def.type_def
			if next_type.category ~= "struct" or next_type.name ~= sub_name then
				error("circular_list next_field '" .. type_def.next_field .. "' must be a weak_ptr to struct '" .. sub_name .. "'")
			end
