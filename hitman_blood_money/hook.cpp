...
GameHook hitman_blood_money::hook(Version version) {
    return [version](std::shared_ptr<void> handle, const BasePtrs& base_ptrs) {
        ...
        const int32_t source_offset
            = version == Version::Steam ? 0x2AF230 : 0x2AFAF0;
        const auto game_stats_ptr
            = base_ptr + (version == Version::Steam ? 0x5B2538 : 0x5B3B38);
        const int32_t offset_e5110
            = version == Version::Steam ? 0xE5110 : 0xE5BE0;
        const int32_t offset_41F83c
            = version == Version::Steam ? 0x41F83C : 0x42083C;
        const int32_t offset_256a10
            = version == Version::Steam ? 0x256A10 : 0x257170;
        const int32_t offset_256a60
            = version == Version::Steam ? 0x256A60 : 0x2571C0;
        const int32_t offset_2ae000
            = version == Version::Steam ? 0x2AE000 : 0x2AE880;
        return install_hook(
            handle,
            {
                Source{
                    base_ptr + source_offset,
                    // mov eax,fs:[0]
                    {0x64, 0xA1, 0, 0, 0, 0},
            ...
            },
            {
                ...
                Code{0x50},                         // push eax
                Jump{
                    Code{0xE8}, Ptr{base_ptr + offset_e5110}
                ...
                Code{0x55},                          // push ebp
                Jump{
                    Code{0xE8}, Ptr{base_ptr + offset_256a10}
                },  // call HitmanBloodMoney.exe+256A10
                Code{0x8B, 0x8E, 0xC4, 0x02, 0, 0},  // mov ecx,[esi+2C4]
                Jump{
                    Code{0xE8}, Ptr{base_ptr + offset_256a60}
                ...
                Jump{
                    Code{0xE8}, Ptr{base_ptr + offset_2ae000}
